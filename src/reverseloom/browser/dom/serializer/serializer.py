...
from reverseloom.browser.dom.serializer.clickable_elements import ClickableElementDetector
from reverseloom.browser.dom.serializer.paint_order import PaintOrderRemover
...
class DOMTreeSerializer:
	...
	def __init__(
		...
		self._xpath_map: DOMXPathMap = {}
		self._previous_cached_xpath_map = previous_cached_state.xpath_map if previous_cached_state else None
		# Add timing tracking
	...
	def serialize_accessible_elements(self) -> tuple[SerializedDOMState, dict[str, float]]:
		...
		return SerializedDOMState(_root=filtered_tree, xpath_map=self._xpath_map), self.timing_info
	...
	def _add_compound_components(self, simplified: SimplifiedNode, node: EnhancedDOMTreeNode) -> None:
		...
		if element_type == 'input':
			...
			node.compound_children.extend(base_components)
			simplified.is_compound_component = True
	...
	def _extract_select_options(self, select_node: EnhancedDOMTreeNode) -> dict[str, Any] | None:
		...
		if not select_node.children_nodes:
			return None
		...
		for child in select_node.children_nodes:
			extract_options_recursive(child)
	...
	def _assign_interactive_indices_and_mark_new_nodes(self, node: SimplifiedNode | None) -> None:
		...
		if not node.excluded_by_parent and not node.ignored_by_paint_order:
			...
			# CUSTOM FOR SCRAPING: Also make elements with direct text or media interactive (index them for bounding boxes)
			has_direct_text = any(c.node_type == 3 and c.node_value and c.node_value.strip() for c in node.original_node.children_nodes)
			is_visual_tag = node.original_node.tag_name in {'img', 'svg', 'picture', 'video', 'canvas'}

			# Containers that render a real background-image and are large enough
			# to be a content surface (captcha puzzle backgrounds, hero banners,
			# poster divs). Without this, a <div style="background-image:url(...)">
			# with no cursor/role/onclick falls through every interactive check
			# and never gets an ocId, so visual_locate has nothing to target.
			# Small decorative icons (<100px) are excluded to avoid flooding the
			# index with sprite chrome.
			has_large_bg_image = False
			snap = node.original_node.snapshot_node
			if snap and snap.computed_styles and snap.bounds:
				bg_img = snap.computed_styles.get('background-image', '')
				if bg_img and bg_img not in ('none', 'initial'):
					if snap.bounds.width >= 100 and snap.bounds.height >= 100:
						has_large_bg_image = True

			is_meaningful_content = has_direct_text or is_visual_tag or has_large_bg_image
			...
			if is_scrollable:
				...
				# CUSTOM FOR SCRAPING: Filter out empty interactive elements (like skeleton loaders)
				if not is_file_input and not is_shadow_dom_element:
					# Check if it has any meaningful content
					# A node is meaningful if it has direct text OR any of its descendants have direct text
					def has_any_text(n):
						if any(c.node_type == 3 and c.node_value and c.node_value.strip() for c in n.children_nodes):
							return True
						return any(has_any_text(c) for c in n.children_nodes)
						
					has_text = has_any_text(node.original_node)
					has_semantic = False
					attrs = node.original_node.attributes or {}
					...
					has_bg_img = False
					if node.original_node.snapshot_node and node.original_node.snapshot_node.computed_styles:
						bg_img = node.original_node.snapshot_node.computed_styles.get('background-image', '')
						if bg_img and bg_img != 'none' and bg_img != 'initial':
							has_bg_img = True
							
					tag = node.original_node.tag_name.lower()
					is_native_visual = tag in {'img', 'svg', 'input', 'textarea', 'select', 'button', 'picture', 'video', 'canvas'}
					
					if not has_text and not has_semantic and not has_bg_img and not is_native_visual:
						should_make_interactive = False
						
			elif is_meaningful_content and is_visible:
				# CUSTOM FOR SCRAPING: Make meaningful content interactive (indexed) if visible
				should_make_interactive = True

			# Add to xpath map if element should be interactive
			if should_make_interactive:
				# Mark node as interactive
				node.is_interactive = True
				# Store backend_node_id in xpath map (model outputs backend_node_id)
				self._xpath_map[node.original_node.backend_node_id] = node.original_node
				self._interactive_counter += 1

				# Mark compound components as new for visibility
				if node.is_compound_component:
					node.is_new = True
				elif self._previous_cached_xpath_map:
					# Check if node is new for regular elements
					previous_backend_node_ids = {node.backend_node_id for node in self._previous_cached_xpath_map.values()}
	...
	def _should_exclude_child(self, node: SimplifiedNode, active_bounds: PropagatingBounds) -> bool:
		...
		# 1.5. CUSTOM FOR SCRAPING: Never exclude elements with direct text or images
		has_direct_text = any(c.node_type == 3 and c.node_value and c.node_value.strip() for c in node.original_node.children_nodes)
		if has_direct_text:
			return False
		if child_tag in ['img', 'svg', 'picture', 'video', 'canvas']:
			return False
	...
	def serialize_tree(node: SimplifiedNode | None, include_attributes: list[str], depth: int = 0) -> str:
		...
		if node.original_node.node_type == NodeType.ELEMENT_NODE:
			...
			if (
				...
				if node.original_node.compound_children:
					compound_info = []
					for child_info in node.original_node.compound_children:
						parts = []
						if child_info['name']:
