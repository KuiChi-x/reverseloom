...
from reverseloom.browser.dom.views import SimplifiedNode
    ...
    def calculate_paint_order(self) -> None:
        ...
        for paint_order, nodes in sorted(grouped_by_paint_order.items(), key=lambda x: -x[0]):
            ...
            for node in nodes:
                ...
                # don't add to rect_union if the element passes through pointer events (click-through)
                ...
                if ((
                        ...
                        < 0.95  # this is highly vibes based number
                )
                ):
                    continue
