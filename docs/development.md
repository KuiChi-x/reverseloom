...
```powershell
.\.venv\Scripts\python.exe -m pip install -e ".[dev,crawler]"
```
...
| extra | 内容 |
| --- | --- |
| 默认依赖 | 本体运行时（含 PyPI 上的 `graphloom`、`patchright` 等） |
| `dev` | pytest、Ruff、PyInstaller |
| `crawler` | agent 生成爬虫常用库（bs4、parsel、pycryptodome…） |

若还要 Postgres checkpoint，再加 `postgres`：`pip install -e ".[dev,crawler,postgres]"`。

如果本机同时开发 graphloom，可覆盖为可编辑安装：
...
./.venv/bin/python -m pip install -e ".[dev,crawler]"
```
